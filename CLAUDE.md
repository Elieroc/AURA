...
**Restructuration install-en-une-commande (2026-08-07)** : plus de 4 stacks
docker-compose séparées (`wazuh/`, `ai/`, `iris/`, `shuffle/`, chacune son
`.env`). Un seul `docker-compose.yml` + un seul `.env` à la racine pilotent
tout ; le code de chaque stack a déménagé sous `src/<stack>/` (ex.
`wazuh/config/...` devient `src/wazuh/config/...`). Les bases de données
(Postgres soc-agent/IRIS, OpenSearch Shuffle/Wazuh) sont en bind mount vers
`db/` (gitignoré), le reste des volumes docker (`wazuh_etc`, `iris_downloads`,
etc.) est inchangé. `mcp/` (serveur MCP Wazuh) reste hors dépôt, hors de ce
compose. **Conséquence prod** : le prochain déploiement sur `/opt/AURA` devra
relancer avec `docker compose up -d --force-recreate` depuis la nouvelle
racine — les bind mounts des conteneurs déjà créés pointent encore vers les
anciens chemins `wazuh/config/...` (sans `src/`), pas seulement vers l'ancien
`/opt/soc-ai`.
...
- **Modèle : API DeepSeek** (`llm.py`, compatible OpenAI). Un seul chemin, pas de repli local : l'IA locale (llama.cpp + Qwen3-8B) a été abandonnée faute de GPU et de RAM sur cet hôte, et tout a été supprimé — binaires, poids, dossier `src/ai/bench/`. Ne pas le réintroduire sans en reparler ; les mesures de l'époque restent dans l'historique git si besoin.
  ...
  - **Règles locales : un fichier par règle, tout en anglais** (descriptions ET commentaires — les descriptions partent dans les alertes, les cases IRIS et le contexte LLM), dans `src/wazuh/config/wazuh_cluster/rules/<id>-<slug>.xml` (plus de `local_rules.xml`). Le répertoire est monté **directement** sur `/var/ossec/etc/rules` : le mécanisme `/wazuh-config-mount` copie sans jamais supprimer, donc un fichier renommé survivait dans le volume et la règle était chargée deux fois, en silence. Chargement par ordre alphabétique = ordre d'identifiant ; l'ordre n'arbitre qu'entre règles **sœurs indépendantes**, à rendre mutuellement exclusives par leurs conditions (`audit.key`, un champ discriminant), jamais par leur position. Avant de reformuler une description, vérifier qu'aucun code ne la matche (`grep -rn rule_desc src/ai/soc_agent/`) : `correlate.py` filtre les graines d'incident sur du texte anglais. Détail : `src/wazuh/config/wazuh_cluster/rules/README.md`.
  - **Capteur auditd** — les règles auditd de l'agent sont versionnées dans `src/wazuh/config/agent/zz-audit-wazuh.rules`. Le préfixe `zz-` est obligatoire (`augenrules` concatène en collation C et le `audit.rules` Debian commence par `-D`). Le fichier se termine par `-e 2` : la conf d'audit est **immuable**, donc **toute modification exige un reboot de la machine** — `augenrules --load` échoue. C'est ce qui empêche systemd-journald (et un attaquant) de reposer `audit_enabled=0`. Revue complète des règles High/Critical, pièges et mesures : `src/wazuh/DETECTION-REVIEW.md`.
...
## Pipeline (src/ai/soc_agent/)

Phase 1 en place : ingestion + corrélation, **sans LLM**. Détail et justifications dans `src/ai/README.md`.
...
- **Filtre VirusTotal des exécutables** (`vt.py`, entre ingest et correlate) : une alerte portant le hash d'un exécutable (Sysmon, FIM, intégration VT) est confrontée à la réputation VT ; un binaire **légitime** (0 moteur positif, hash connu de VT) est marqué `suppressed` → n'ouvre ni ne rejoint un case (un exe propre seul n'ouvre pas de dossier). Déterministe, pas le LLM. Portée étroite : uniquement les exécutables **déposés hors répertoires système** (un LOLBin propre de System32 reste analysé — détection comportementale). Dans le doute (hash inconnu/vu de trop peu de moteurs) : pas de suppression. Cache `vt_file_reputation` (TTL) + plafond d'appels (`VT_MAX_LOOKUPS`, API publique 4/min) ; clé réutilisée de l'intégration VT Wazuh (`VT_API_KEY`), sans clé le filtre est inactif.
...
**Mode training** (`soc_agent.training`, conteneur `soc-training`) : fenêtre de mise en service sur un SI déjà en production, ouverte au premier lancement par l'administrateur (`TRAINING_*` dans le `.env` racine, lu directement par `docker compose up -d`). Pendant `TRAINING_DAYS` (7), **le pipeline d'analyse est suspendu** — `cycle.py` ingère puis s'arrête (`training.en_cours`), donc ni triage, ni case, ni remédiation — et toute alerte ≥ niveau 12 devient une exception `whitelist_rules` (`source='training'`), **sans LLM** : groupement par (règle, machine), signature qui contient **toujours** `agent_name` (le bruit ambiant appartient à un hôte) et tolère l'absence de discriminant, contrairement à la whitelist auto. Plafond propre : `TRAINING_MAX_LEVEL` (15, contre 14) — le bruit CRITICAL est celui qui déclenche les remédiations les plus coûteuses. Clôture dans l'ordre : réapplication du noise filter à l'existant → case IRIS « TRAINING » (une tâche par exception) → statut `finished`, qui seul débloque le pipeline (pas la date : sinon un cycle corrélerait le backlog non filtré). Révocation : tâche passée en `Canceled` → exception désactivée + filtre réappliqué, sens unique. Limite assumée : une intrusion en cours au lancement est apprise comme du bruit — le case TRAINING est le contrôle qui la rattrape.

Déclenchement **périodique** : `soc_agent.cycle` enchaîne ingest → correlate → triage → whitelist → cases IRIS ; conteneur `soc-agent-cycle` (boucle shell, définie dans le `docker-compose.yml` racine) toutes les 5 min. Verrou consultatif Postgres anti-chevauchement. Plus lancé à la main. Même schéma (conteneur + boucle + verrou) pour `soc-agent-reconcile` (1 min, annule une remédiation dont la tâche IRIS passe en `Canceled`) et `soc-agent-whitelist-task` (1 min, traite les tâches IRIS WHITELIST passées en `To do`).
