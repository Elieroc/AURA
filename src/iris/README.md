...
#   openssl rand -hex 32
# Puis décommenter et remplir IRIS_ADM_PASSWORD et IRIS_API_KEY.
$EDITOR .env

./src/iris/scripts/generate-certs.sh      # PKI locale (demande sudo, cf. plus bas)
docker compose up -d
```
...
> depuis l'UI, soit repartir de zéro. La base Postgres d'IRIS vit désormais en
> bind mount (`db/iris-postgres/`, hors des named volumes) : `docker compose
> down -v` ne la détruit plus (elle ne supprime que `iris_downloads`,
> `user_templates`, `server_data`, et fait de même pour les autres stacks du
> compose racine). Pour vraiment repartir de zéro sur IRIS : `docker compose
> down` puis `rm -rf db/iris-postgres/*`.

## Vérification

```bash
set -a; source .env; set +a   # depuis la racine du dépôt
curl -sk -H "Authorization: Bearer ${IRIS_API_KEY}" https://127.0.0.1:8443/api/ping
...
sans qu'on l'ait décidé, et les migrations sont irréversibles. Sauvegarder
`db/iris-postgres/` avant toute montée de version.
...
Authentification par `Authorization: Bearer <IRIS_API_KEY>`. La plupart des
...
docker compose restart iris-worker

# Sauvegarde de la base (avant montée de version)
docker exec iris-db pg_dump -U postgres iris_db | gzip > iris-$(date +%F).sql.gz
```

`docker compose down -v` supprime les named volumes (`iris_downloads`,
`user_templates`, `server_data`, et l'équivalent pour Wazuh/Shuffle) mais PAS
les cases : la base Postgres d'IRIS vit en bind mount (`db/iris-postgres/`),
donc elle survit à `-v`. Pour vraiment détruire les cases : `docker compose
down` puis `rm -rf db/iris-postgres/*`.
