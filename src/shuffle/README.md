...
attendues par le `.env` racine (`SHUFFLE_WEBHOOK_ISOLATE` / `_KILL`), pour que
`mitigate.py` retrouve la même URL quelle que soit l'instance Shuffle.

```bash
# Depuis src/shuffle/ — .env est désormais à la racine du dépôt (../../.env).
...
Scripts active response déployés sur l'agent Linux (voir `src/wazuh/active-response/`) :
