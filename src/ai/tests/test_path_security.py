...
from soc_agent.mitigate import (_win_path_outside_system, _win_path_suspicious,
                                _norm_win_path, _win_suspicious_files)
...
def test_norm_preserve_les_chemins_unc():
    ...
    assert _norm_win_path(r"\\?\UNC\srv\partage\implant.exe") == \
...
def test_system32_reste_protege_quelle_que_soit_l_ecriture():
    ...
    for variant in (
        ...
        assert not _win_path_outside_system(variant), variant
        assert not _win_path_suspicious(variant), variant
...
def test_chemin_non_resolvable_refuse():
    ...
    assert not _win_path_outside_system(r"..\..\evil.exe")
    assert not _win_path_outside_system("")
...
def test_un_vrai_implant_reste_ciblable():
    ...
    assert _win_path_suspicious(r"C:\Users\Public\evil.exe")
    assert _win_path_suspicious(r"\\?\C:\Users\Public\evil.exe")
    assert _win_path_outside_system(r"C:\ProgramData\payload.dll")
...
def test_extraction_normalise_les_cibles():
    ...
    alert = {"agent_id": "001", "entity": None, "raw": {"data": {"win": {
        "eventdata": {"image": r"\\?\C:\Users\Public\evil.exe"}}}}}
    assert _win_suspicious_files([alert]) == {r"C:\Users\Public\evil.exe"}
...
def test_extraction_ecarte_system32_deguise():
    ...
    assert _win_suspicious_files([alert]) == set()
