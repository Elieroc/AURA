...
from soc_agent.rule_tuning import _commentable, build_xml
...
def test_url_hostile_ne_produit_pas_de_regle_supplementaire():
    ...
    xml = _build({"rule_id": "31100", "url": CHARGE})
    assert xml is not None

    root = ElementTree.fromstring(xml)
    rules = root.findall("rule")
    assert len(rules) == 1, "une regle injectee s'est ajoutee au fichier"
    assert rules[0].get("id") == "101000"
    assert rules[0].get("level") == "5"
    ...
    assert "<" not in header[4:] and ">" not in header[4:]
...
def test_le_xml_genere_reste_bien_forme_sur_valeur_hostile():
    ...
    for value in (CHARGE, "a--b", "<!--", "-->", "]]>", "&amp;", "'\"<>"):
        xml = _build({"rule_id": "31100", "url": value})
        assert xml is not None, value
...
def test_caractere_interdit_par_xml_refuse_la_signature():
    ...
    assert _build({"rule_id": "31100", "url": "/x\x00nul"}) is None
    assert _build({"rule_id": "31100", "command": "a\x07b"}) is None
...
def test_la_condition_reste_ancree_et_echappee():
    ...
    xml = _build({"rule_id": "31100", "url": "/build.sh"})
    field = ElementTree.fromstring(xml).find("rule/url")
    assert field.get("type") == "pcre2"
    assert field.text == r"^/build\.sh$"
...
def test_signature_canonique_relisible_pour_l_idempotence():
    ...
    xml = _build(signature)
    from soc_agent.whitelist import _canonical
    expected = _commentable(_canonical(signature), 400)
    assert f"signature-canonique: {expected}" in xml
