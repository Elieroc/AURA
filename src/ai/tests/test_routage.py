...
from soc_agent import config, routage
...
def test_generique_hors_vocabulaire_est_refuse():
    ...
    """
    assert routage._valider("generique", "pfsense", {"pfsense"}) is not None
    assert routage._valider("generique", "fortinet", {"fortinet"}) is not None
    assert routage._valider("generique", "firewall", set()) is None
...
def test_application_doit_etre_attestee_par_les_donnees():
    ...
    assert routage._valider("applicative", "jellyfin", {"jellyfin", "media"}) is None
    assert routage._valider("applicative", "grafana", {"jellyfin"}) is not None
...
def test_nom_de_metier_refuse_comme_application():
    ...
    assert routage._valider("applicative", "web", {"web"}) is not None


def test_formes_invalides():
    for suffixe in ("", "a", "Web", "web-proxy", "proxy2", "x" * 21):
        assert routage._valider("generique", suffixe, set()) is not None
...
def test_suffixes_reserves_par_la_stack():
    ...
    for suffixe in ("alerts", "monitoring", "statistics", "ai", "voc"):
        assert routage._valider("generique", suffixe, set()) is not None


def test_inconnu_nest_pas_un_nom():
    assert routage._valider("inconnu", "", set()) is not None


def test_repli_reste_deterministe_et_conforme():
    r = routage._repli({"critere_valeur": "npm-access"}, "motif")
    assert r["index_base"] == "wazuh-npmaccess"
    assert r["nomme_par"] == "repli"      # -> jamais auto-appliqué
...
ROUTES = [
    {"critere_type": "decoder", "critere_valeur": "npm-access",
     "index_base": "wazuh-proxy"},
    {"critere_type": "groups", "critere_valeur": "adguard",
...
def test_script_appris_teste_le_decodeur_avant_les_groupes():
    ...
    src = routage._script_appris(ROUTES)["script"]["source"]
...
def test_rendu_stable_octet_pour_octet():
    ...
    a = json.dumps(routage._script_appris(ROUTES), sort_keys=True)
    b = json.dumps(routage._script_appris(list(ROUTES)), sort_keys=True)
    assert a == b
...
def test_valeurs_non_conformes_refusees_avant_de_generer_du_painless():
    ...
    for mauvais in ("npm'access", "a b", "x" * 65, "év", ""):
        with pytest.raises(ValueError):
            routage._script_appris([{"critere_type": "decoder",
                                     "critere_valeur": mauvais,
                                     "index_base": "wazuh-proxy"}])
...
def test_insertion_apres_le_routage_statique():
    ...
    rendu = routage.rendre(PIPELINE, ROUTES)
    tags = [next(iter(p.values())).get("tag") for p in rendu["processors"]]
    assert tags.index(routage.TAG_APPRIS) > tags.index(routage.TAG_STATIQUE)
    assert len(rendu["processors"]) == len(PIPELINE["processors"]) + 1
...
def test_le_script_yara_garde_le_dernier_mot():
    ...
    rendu = routage.rendre(PIPELINE, ROUTES)
    dernier = next(iter(rendu["processors"][-1].values()))
    assert "YARITRUST" in dernier["description"]
...
def test_insertion_par_defaut_sur_la_description():
    ...
    assert routage._position_insertion(sans_tag["processors"]) == 2
...
def test_refus_d_inserer_a_l_aveugle():
    ...
    with pytest.raises(RuntimeError):
        routage.rendre({"processors": [{"json": {}}]}, ROUTES)
...
def test_le_processor_appris_est_retirable():
    ...
    rendu = routage.rendre(PIPELINE, ROUTES)
    assert routage._sans_appris(rendu)["processors"] == PIPELINE["processors"]
    assert routage._sans_appris(PIPELINE)["processors"] == PIPELINE["processors"]
...
def test_message_repose_le_prefixe_efface_a_l_indexation():
    ...
    m = routage._message({"timestamp": "2026-08-14T10:00:00.000+0000"})
    assert m["fields"]["index_prefix"] == routage.PREFIXE_DEFAUT
    deja = {"fields": {"index_prefix": "autre-"}}
    assert routage._message(deja)["fields"]["index_prefix"] == "autre-"


def test_simulation_sans_temoin_ne_bloque_rien():
    assert routage.simuler({}, []) == []
...
def test_lecture_d_une_simulation(monkeypatch):
    ...
    monkeypatch.setattr(routage, "_indexer", lambda *a, **k: Faux())
    cas = [{"source_key": "decoder:npm-access", "index_base": "wazuh-proxy",
            "exemple": {}},
           {"source_key": "decoder:jellyfin", "index_base": "wazuh-jellyfin",
            "exemple": {}},
           {"source_key": "groups:adguard", "index_base": "wazuh-dns",
            "exemple": {}}]
    echecs = routage.simuler({}, cas)
    assert len(echecs) == 2
    assert "attendu wazuh-jellyfin, obtenu wazuh-alerts-4.x" in echecs[0]
    assert "PERDU" in echecs[1]


def test_base_index_retire_la_date():
    assert routage._base_index("wazuh-linux-2026.08.14") == "wazuh-linux"
    assert routage._base_index("wazuh-voc-vulns") == "wazuh-voc-vulns"
...
def test_le_bruit_transverse_nest_pas_une_source():
    ...
    for d in ("ossec", "rootcheck", "sca", "wazuh"):
        assert d in routage.DECODEURS_TRANSVERSES
    for g in ("syscheck", "sca", "virustotal", "vulnerability-detector"):
        assert g in routage.GROUPES_TRANSVERSES
...
def test_windows_eventchannel_nest_pas_traite_comme_ambigu():
    ...
    assert "windows_eventchannel" not in routage.DECODEURS_AMBIGUS
    assert "json" in routage.DECODEURS_AMBIGUS
...
def test_plafond_de_creation_est_bas():
    ...
    assert 1 <= config.ROUTAGE_MAX_NOUVEAUX_PAR_JOUR <= 3
...
def test_seuil_de_silence_est_en_jours_pas_en_minutes():
    ...
    assert config.ROUTAGE_SILENCE_HEURES >= 24
