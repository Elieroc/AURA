...
_TITRE_PREFIXE = "TRAINING — whitelist"
_STATUT_POSE = "Done"
_STATUT_ANNULE = "Canceled"
...
def run_en_cours(conn) -> dict | None:
    ...
    """
    return conn.execute(
        "SELECT id, started_at, ends_at, jours, status, iris_case_id "
        "  FROM training_runs WHERE status = 'running' "
        " ORDER BY id DESC LIMIT 1").fetchone()


def en_cours(conn) -> bool:
...
def demarrer(conn, jours: int) -> dict | None:
    """Ouvre une fenêtre de training. None si une fenêtre est déjà ouverte."""
    if run_en_cours(conn):
        return None
    run = conn.execute("""
        INSERT INTO training_runs (ends_at, jours)
        VALUES (now() + make_interval(days => %s), %s)
        RETURNING id, started_at, ends_at, jours, status, iris_case_id
    """, (jours, jours)).fetchone()
    conn.commit()
    log.info("training : fenêtre #%s ouverte pour %d jour(s), fin %s",
             run["id"], jours, run["ends_at"])
    return run
...
def _signature_training(alertes_raw: list[dict], rule_id: str,
...
def apprendre(conn, run: dict) -> list[dict]:
    ...
    """
    lignes = conn.execute(SELECT_ALERTES, {
    ...
    for l in lignes:
        groupes.setdefault((l["rule_id"], l["agent_name"]), []).append(l)

    existantes = {r["signature"] for r in conn.execute(
        "SELECT signature FROM whitelist_rules").fetchall()}

    creees: list[dict] = []
    for (rule_id, agent_name), groupe in sorted(
            groupes.items(), key=lambda kv: (kv[0][0], kv[0][1] or "")):
        niveau = max(l["rule_level"] for l in groupe)
        if niveau > config.TRAINING_MAX_LEVEL:
            log.info("training : niveau %d > %d, règle %s@%s non whitelistée",
                     niveau, config.TRAINING_MAX_LEVEL, rule_id, agent_name)
            continue
        raws = [l["raw"] if isinstance(l["raw"], dict) else json.loads(l["raw"])
                for l in groupe]
        signature = _signature_training(raws, rule_id, agent_name)
        canon = _canonique(signature)
        if canon in existantes:
            continue

        desc = next((l["rule_desc"] for l in groupe if l["rule_desc"]), "")
        reason = (f"Bruit ambiant appris en training (fenêtre #{run['id']}, "
                  f"{len(groupe)} alerte(s) niveau max {niveau}) — "
                  f"{desc or canon}")
        conn.execute("""
        ...
        """, (canon, json.dumps(signature), reason, len(groupe), run["id"]))
        conn.commit()
        existantes.add(canon)
        creees.append({"signature": canon, "match_all": signature,
                       "alertes": len(groupe), "niveau": niveau,
                       "rule_desc": desc})
...
def _description_case(run: dict, regles: list[dict]) -> str:
    debut = run["started_at"].strftime("%Y-%m-%d %H:%M")
    fin = run["ends_at"].strftime("%Y-%m-%d %H:%M")
    return (
...
def _description_tache(regle: dict) -> str:
    ...
    return (
        ...
        f"```json\n"
        f"{json.dumps(regle['match_all'], ensure_ascii=False, indent=2)}\n"
        f"```\n\n"
        f"{regle['reason']}\n\n"
...
def cloturer(conn, run: dict) -> dict:
    ...
    supprimees, vus = ingest.reappliquer_filtre()
    log.info("training : filtre réappliqué, %d/%d alertes supprimées",
             supprimees, vus)

    case_id = None
    try:
        case_id = _creer_case_iris(conn, run, regles)
    ...
    if case_id is None:
        return {"case_id": None, "regles": len(regles),
    ...
    log.info("training : fenêtre #%s clôturée -> case IRIS #%s (%d exceptions)",
             run["id"], case_id, len(regles))
    return {"case_id": case_id, "regles": len(regles),
            "supprimees": supprimees}


def _creer_case_iris(conn, run: dict, regles: list[dict]) -> int:
    ...
    case = _client()
    debut = run["started_at"].strftime("%Y-%m-%d")
    r = case.add_case(
        case_name=f"TRAINING — bruit ambiant du SI ({debut}, "
                  f"{run['jours']} j)",
    ...
    for regle in regles:
        titre = f"{_TITRE_PREFIXE} : {regle['signature']}"
        try:
            rt = case.add_task(
                title=titre[:250],
                status=_STATUT_POSE,
                assignees=[],
                description=_description_tache(regle),
...
def reconcilier(conn) -> list[dict]:
    ...
    if not actives:
        return []
    par_task = {r["iris_task_id"]: r for r in actives}
